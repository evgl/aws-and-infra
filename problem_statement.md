# DevOps Interview Assessment – Problem Statement  
...
- **README.md:** Describe architecture, AWS region used, deployment steps, and how to run the verification script. Include a diagram (hand-drawn or generated).
- **docker-compose.yml:** File used on EC2 instances.
- **verify_endpoints.sh** or **verify_endpoints.py:** Health check script.
- **Screenshots or command output** showing:
  - ECR repository images
  - `docker ps` on both EC2 instances
  - ALB DNS + curl responses
  - (Bonus) ASG scale-out event evidence
- **Cleanup confirmation:** Note stating all AWS resources have been torn down.
- **(Optional) Terraform code** if infrastructure was provisioned via IaC.
...
This assignment may incur small AWS costs (typically under $30).  
If needed, up to $35 (50,000 KRW) will be reimbursed.  

Please keep receipts and usage screenshots if claiming reimbursement.